...
def get_process_map(img, tilesize, masks, reportfile):
    ...
    nrow, ncol = [img.shape[0] / tilesize, img.shape[1] / tilesize]
...
def preprocessing(**kwargs):
    ...
    img = read_region(wsi, (0, 0), wsi.level_count - 1,
...
def tile_scale(**kwargs):
    ...
    if wsi.properties['aperio.AppMag'] == '20':
        ...
        level_20 = 1
    nrow, ncol = [
        wsi.level_dimensions[level_20][0] / tilesize,
        wsi.level_dimensions[level_20][1] / tilesize
    ]
    tilemap = kwargs['process_map']
    tilemap.dtype = np.uint8
    tilemap = cv2.resize(
        tilemap, dsize=(nrow, ncol), interpolation=cv2.INTER_NEAREST)
    ...
    tilemap_name = os.path.join(
        kwargs['exp_home'], 'data_tilemap_{}.npy'.format(kwargs['tilesize']))
...
def process_scale(**kwargs):
    ...
    expdirs = [
        os.path.join(kwargs['exp_home'], 'tiles_{}'.format(kwargs['scale']))
    ]
    for output in kwargs['outputs']:
        d = os.path.join(kwargs['exp_home'], 'prob_{}_{}'.format(
            output, kwargs['scale']))
        if os.path.exists(d):
            expdirs.append(d)
        else:
...
def generate_stats(**kwargs):
    ...
    if process_map.shape[:2] != labels.shape[:2]:
        ...
        process_map = cv2.resize(
            process_map,
            dsize=labels.shape[:2],
            interpolation=cv2.INTER_NEAREST)
    ...
    # Total cancer area
    cancer_area = np.add((labels == 1).sum(), (labels == 2).sum())
...
def draw_class_images(classimg, exp_home):
    ...
    for k in range(nclass):
        c = classimg[:, :, k]
        c = cv2.resize(
            c, dsize=(0, 0), fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR)
...
=======
        'Slide\nName', 'Processing\nTime (s)', 'Date', 'Tumor %',
>>>>>>> 869bff902c83fb7256f9c36d9946ddb303295161:core/histo_pipeline.py
        'Major\nPatterns'
    ]
...
=======
            pattern_str = r'{} {} $({:3.2f})$\%'.format(
                pattern_str, pattern_dict[k], ga / float(cancer_area) * 100)
>>>>>>> 869bff902c83fb7256f9c36d9946ddb303295161:core/histo_pipeline.py
...
=======
        slide_name, '${}min$ ${:3.1f}s$'.format(int(time_min), time_sec),
>>>>>>> 869bff902c83fb7256f9c36d9946ddb303295161:core/histo_pipeline.py
        r'{}'.format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        r'${:3.2f}$\%'.format(Tumor_pct * 100), pattern_str
    ]
...
def main(**kwargs):
    ...
    process_map = preprocessing(
        filename=kwargs['filename'], reportfile=reportfile)
...
if __name__ == '__main__':
    # Take in or set args
    # TODO implement passed in args or loading arguments form an external file.
    # This function should definitely not have anything hard-coded.
    scales = [384, 512, 896]
    scale_weights = [3, 0.5, 0.25]
    # scales = [384, 896]
    # scale_weights = [3, 0.25]
    weights = [
        '/home/nathan/semantic-pca/weights/seg_0.8.1/norm_resumed_iter_32933.caffemodel',
    ...
    writeto = '/home/ingn/semantic-pca/
    outputs = [0, 1, 2, 3, 4]
